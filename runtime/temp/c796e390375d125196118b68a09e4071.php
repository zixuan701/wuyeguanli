<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:80:"D:\phpStudy\WWW\tp5\public/../application/admin/view/default/renzheng\index.html";i:1534055773;s:77:"D:\phpStudy\WWW\tp5\public/../application/admin/view/default/public\base.html";i:1496373782;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
...
<div class="cf">
    ...
    <!--<a class="btn" title="删除" href="<?php echo url('del'); ?>" target-form="ids">删 除</a>-->
...
<div class="data-table table-striped">
    <table>
        ...
        <tbody>
        <?php if(!(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty()))): if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rezheng): $mod = ($i % 2 );++$i;?>
        <tr>
            <td><input class="ids row-selected" type="checkbox" name="id[]" id="" value="<?php echo $repair['id']; ?>"></td>
            <td><?php echo $rezheng['id']; ?></td>
            <td><?php echo $rezheng['name']; ?></td>
            <td><?php echo $rezheng['num']; ?></td>
            <td><?php echo $rezheng['guanxi']; ?></td>
            <td><?php echo $rezheng['tel']; ?></td>
            <td><?php echo $rezheng['ids']; ?></td>
            <td><?php echo !empty($rezheng['status']) && $rezheng['status']==1?'已审核':'待审核'; ?></td>
            <td>
                <!--<a title="编辑" href="<?php echo url('edit?id='.$repair['id']); ?>">编辑</a>-->

                <!--&lt;!&ndash;<a href="<?php echo url('setStatus?ids='.$channel['id'].'&status='.abs(1-$channel['status'])); ?>" class="ajax-get"><?php echo show_status_op($channel['status']); ?></a>&ndash;&gt;-->

                <!--<a class="confirm ajax-get" title="删除" href="<?php echo url('del?id='.$repair['id']); ?>">删除</a>-->



                <?php if($rezheng['status']==1): ?>
                <a href=""><button class="btn btn-success btn-sm" disabled title="该资料已审核通过">已审核</button></a>
                <?php else: ?>
                <a href="<?php echo url('shenhe?id='.$rezheng['id']); ?>"><button class="btn btn-success btn-sm" title="点击审核资料">审&emsp;核</button></a>
                <?php endif; ?>
...
<div>
    <ul>
        <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rezheng): $mod = ($i % 2 );++$i;?>
        <li> <?php echo $rezheng['nickname']; ?></li>
        <?php endforeach; endif; else: echo "" ;endif; ?>
    </ul>
</div>
